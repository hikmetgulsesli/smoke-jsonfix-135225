...
export function SifirlamaOnayModali({ count, onConfirm, onCancel }: SifirlamaOnayModaliProps) {
  return (
    <>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none opacity-30 filter blur-sm transition-all duration-300">
        <div className="flex flex-col items-center justify-center space-y-12">
          <h1 className="text-[8rem] font-black tracking-[-0.02em] text-primary leading-none">{count}</h1>
        ...
        <div className="bg-surface-container-low rounded-2xl w-full max-w-sm overflow-hidden flex flex-col shadow-[0_32px_64px_rgba(11,19,38,0.8),0_0_0_1px_rgba(218,226,253,0.05)] relative transform transition-all scale-100 opacity-100">
          ...
          <div className="p-8 pb-6 flex flex-col items-center text-center relative z-10">
            ...
            <div className="flex flex-col w-full space-y-3">
              <button onClick={onConfirm} className="w-full bg-error text-on-error font-label font-bold text-xs tracking-widest uppercase py-4 rounded-xl hover:bg-error/90 active:scale-[0.98] transition-all shadow-[0_8px_16px_rgba(255,180,171,0.1)] flex items-center justify-center space-x-2 cursor-pointer">
                <span className="material-symbols-outlined text-sm" style={{fontVariationSettings: "'FILL' 1"}}>delete</span>
                <span>EVET, SIFIRLA</span>
              </button>
              <button onClick={onCancel} className="w-full bg-transparent text-on-surface font-label font-bold text-xs tracking-widest uppercase py-4 rounded-xl hover:bg-surface-container-highest active:scale-[0.98] transition-all border border-outline-variant/30 cursor-pointer">
                İPTAL
              </button>
            </div>
          </div>
